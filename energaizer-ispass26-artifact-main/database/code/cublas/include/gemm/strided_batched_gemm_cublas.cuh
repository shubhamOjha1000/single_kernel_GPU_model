...
void GPU_fill_rand(float *A, int row, int col, int batch);
...
float gemmstridedbatched_cublas_kernel(nvmlClass &nvml)
{
    // Result matrix C, pointers and other data
    // std::vector<A> matC(gdim_M * gdim_N * gbatch);
    M* dev_matA;
    M* dev_matB;
    A* dev_matC;
    S alpha;
    ...
    gpuErrchk(cudaMalloc((void**)&dev_matA, gdim_M * gdim_K * gbatch * sizeof(M)));
    gpuErrchk(cudaMalloc((void**)&dev_matB, gdim_K * gdim_N * gbatch * sizeof(M)));
    gpuErrchk(cudaMalloc((void**)&dev_matC, gdim_M * gdim_N * gbatch * sizeof(A)));

    initialize_matrix<A><<<((gdim_M*gdim_N*gbatch)+512-1)/512,512>>>(dev_matC, gdim_M*gbatch, gdim_N, 0.0);

    float* rand_matA;
    float* rand_matB;
    gpuErrchk(cudaMalloc((void**)&rand_matA, gdim_M * gdim_K * gbatch * sizeof(float)));
    gpuErrchk(cudaMalloc((void**)&rand_matB, gdim_K * gdim_N * gbatch * sizeof(float)));
    GPU_fill_rand(rand_matA, gdim_M, gdim_K, gbatch);
    GPU_fill_rand(rand_matB, gdim_K, gdim_N, gbatch);

    initialize_random<M><<<((gdim_M*gdim_K*gbatch)+512-1)/512,512>>>(rand_matA, dev_matA, gdim_M*gbatch, gdim_K);
    initialize_random<M><<<((gdim_N*gdim_K*gbatch)+512-1)/512,512>>>(rand_matB, dev_matB, gdim_K, gdim_N*gbatch);
    ...
    // stride 
    long long int strA = gdim_M * gdim_K;
    long long int strB = gdim_N * gdim_K;
    long long int strC = gdim_M * gdim_N;
   
    if (gnum_iter > 1) { 
        for (int warmup=0;warmup<5;warmup++){
            gpuErrchk(cublasGemmStridedBatchedEx(cublasH,                       // handle to cuBLAS library context
                            ...
                            strC,                          // Stride of batched result matrix C,
                            gbatch,                        // batch count in int
    ...
    if(gprint_result)
    {
        ...
        view_matrix_fp<M><<<1,1>>>(dev_matA, gdim_M*gbatch, gdim_K);
        gpuErrchk(cudaDeviceSynchronize());
        std::cout << "Matrix B: " << std::endl;
        view_matrix_fp<M><<<1,1>>>(dev_matB, gdim_K, gdim_N*gbatch);
        gpuErrchk(cudaDeviceSynchronize());
        std::cout << "Matrix C: " << std::endl;
        view_matrix_fp<A><<<1,1>>>(dev_matC, gdim_M*gbatch, gdim_N);
        gpuErrchk(cudaDeviceSynchronize());
    }
...
float gemmstridedbatched_cublas_kernel_double_double_double(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_float_float_float(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_float_half_float(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_float_half_half(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_half_half_half(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_float_bf16_float(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_float_bf16_bf16(nvmlClass &nvml);
float gemmstridedbatched_cublas_kernel_int_int8_int(nvmlClass &nvml);
