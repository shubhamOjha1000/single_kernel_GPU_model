...
#endif
static std::string nvml_filename = "nvml.csv";
...
testResult_t BenchTime(struct threadArgs* args, ncclDataType_t type, ncclRedOp_t op, int root) {
  ...
  // nvmlClass nvml(args->nGpus, args->nvml_filename);

  std::thread threadStart( &nvmlClass::getStats,
                           args->nvml );  // threadStart starts running
  ...
  std::thread threadKill( &nvmlClass::killThread, args->nvml );
  threadStart.join( );
  threadKill.join( );
...
testResult_t TimeTest(struct threadArgs* args, ncclDataType_t type, const char* typeName, ncclRedOp_t op, const char* opName, int root) {
  ...
  do {
    for (size_t size = args->minbytes; size<=args->maxbytes; size = ((args->stepfactor > 1) ? size*args->stepfactor : size+args->stepbytes)) {
      ...
      TESTCHECK(BenchTime(args, type, op, root));
      PRINT("\n");
    }
  } while (--repeat);
...
int main(int argc, char* argv[]) {
  ...
  static struct option longopts[] = {
    ...
    {"nvml_filename", required_argument, 0, 'F'},
    {"in_place", required_argument, 0, 'I'},
    {}
  };

  while(1) {
    int c;
    c = getopt_long(argc, argv, "t:g:b:e:i:f:n:m:w:N:p:c:o:d:r:z:y:T:hG:C:a:R:F:I:", longopts, &longindex);
    ...
    switch(c) {
      ...
      case 'R':
        ...
        break;
      case 'F':
        nvml_filename = std::string(optarg);
        break;
      case 'I':
        in_place = (int)strtol(optarg, NULL, 0);
        break;
      case 'h':
      default:
...
#define MAX_LINE 2048
  ...
  for (int t=nThreads-1; t>=0; t--) {
    ...
    nvmlClass nvml(nGpus, nvml_filename);
    threads[t].args.nvml = &nvml;

    threads[t].args.in_place = in_place;

    threads[t].func = parallel_init ? threadInit : threadRunTests;
    if (t)
      TESTCHECK(threadLaunch(threads+t));
    else
      // printf("Launching thread for test");
