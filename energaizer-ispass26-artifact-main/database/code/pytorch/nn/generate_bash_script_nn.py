...
'''.format(args.sudo_password, args.cuda_device, args.gpu_power_limit))
        ...
        for _, config in workload.iterrows():
            ...
            cmd = '{} pytorch_nn_benchmark.py'.format(args.python_bin_path)
            ...
            cmd += ' --workload_type {}'.format(config['workload_type'])
            cmd += ' --workload_function {}'.format(config['workload_function'])

            dims = eval(config['dims']) if type(config['dims'])==str else config['dims']

            if config['workload_type'] == 'conv':
                cmd += ' --workload_dim {} {} {} {}'.format(dims[0], dims[2], dims[3], dims[4])
                cmd += ' --conv_output_channel_dim {}'.format(dims[1])
                cmd += ' --conv_kernel_size {}'.format(dims[5])
                cmd += ' --conv_stride {}'.format(dims[7])
                cmd += ' --conv_padding {}'.format(dims[8])

                if config['workload_function'] != 'conv2d':
            ...
            # log_save_prefix = 'sdpa_{}_{}_{}_{}_{}_{}'.format(args.precision, args.sdpa_backend, args.sdpa_batch_size, args.sdpa_num_heads, args.sdpa_sequence_len, args.sdpa_embed_dimension)
            log_save_prefix = 'nn_{}_{}_{}'.format(config['workload_type'], config['workload_function'], config['precision'])
            if config['workload_type'] == 'conv':
                log_save_prefix += '_b{}_m{}_c{}_hw{}_rs{}_stride{}_padding{}'.format(dims[0], dims[1], dims[2], dims[3], dims[5], dims[7], dims[8])
            elif config['workload_type'] == 'nonlinear':
                if config['workload_function'].lower() == 'softmax':
                    b = int(np.prod(dims[:-1]))
                    d = dims[-1]
                else:
                    b = dims[0]
                    d = int(np.prod(dims[1:]))
                log_save_prefix += '_b{}_d{}'.format(b, d)
            else:
                d = int(np.prod(dims)) if type(dims) != int else dims
